...
function useVoiceAgent() {
  ...
  const connect = useCallback(async () => {
    if (voiceStateRef.current !== "idle" && voiceStateRef.current !== "error") return;
    ...
    try {
      ...
      socket.on("audio-delta", ({ delta }: { delta: string }) => {
        setVoiceState((prev) => (prev === "active" || prev === "agent-speaking" ? "agent-speaking" : prev));
      ...
      socket.on("transcript-done", ({ transcript }: { transcript: string }) => {
        if (transcript?.trim()) {
          setTranscriptLog((prev) => [...prev, { role: "agent", text: transcript }]);
          agentTranscriptRef.current = "";
          setAgentTranscript("");
          setTimeout(() => setVoiceState((prev) => (prev === "agent-speaking" ? "active" : prev)), 600);
        }
      });
      ...
      socket.on("user-transcript", ({ transcript }: { transcript: string }) => {
        if (transcript?.trim()) {
          ...
          setTranscriptLog((prev) => [...prev, { role: "user", text: transcript }]);
      ...
      socket.on("realtime-error", ({ error }: { error: { message: string } }) => {
...
// ─── Voice Modal ──────────────────────────────────────────────────────────────
function VoiceModal({ onClose, carContext }: { onClose: () => void; carContext?: string }) {
  const {
    voiceState,
    agentTranscript,
    userTranscript,
  ...
  return (
    ...
    >
      ...
      >
        {/* Drag pill */}
        <div style={{ width: 40, height: 3, background: "rgba(255,255,255,0.1)", borderRadius: 2, margin: "12px auto 20px" }} />

        {/* Header */}
        <div style={{ textAlign: "center", marginBottom: 28, padding: "0 24px" }}>
          <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 900, fontSize: 11, letterSpacing: "0.25em", textTransform: "uppercase", color: "#00A8E8", marginBottom: 6 }}>
            BYD Fairfield · Voice Assistant
          </div>
          <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 800, fontSize: 22, letterSpacing: "0.05em", textTransform: "uppercase", color: "#E8ECF0" }}>
        ...
        <div style={{ position: "relative", width: 120, height: 120, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 24 }}>
          {[0, 1, 2].map((i) => (
            <div
              ...
              style={{
                ...
                opacity: voiceState === "active" || voiceState === "agent-speaking" ? 0.5 : 0,
                animation:
          ...
          <button
            ...
            disabled={voiceState === "connecting" || voiceState === "active" || voiceState === "agent-speaking"}
            style={{
              width: 100,
              height: 100,
              borderRadius: "50%",
              ...
              animation: voiceState === "active" ? "pulseGlow 2s ease-in-out infinite" : "none",
              position: "relative",
              zIndex: 1,
            }}
          >
              ...
              <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
                {[0, 1, 2].map((i) => (
                  <div
                    ...
                    style={{ width: 6, height: 6, borderRadius: "50%", background: "white", animationDelay: `${i * 0.18}s` }}
                  />
                ))}
              </div>
            )}
            ...
            {voiceState === "agent-speaking" && (
              <div style={{ display: "flex", gap: 4, alignItems: "center", height: 28 }}>
                {[0, 1, 2, 3, 4].map((i) => (
                  <div key={i} className="voice-bar" style={{ width: 4, height: 20, background: "white", borderRadius: 2 }} />
                ))}
              </div>
            )}
          </button>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 20 }}>
          <div style={{ width: 7, height: 7, borderRadius: "50%", background: statusColor[voiceState], flexShrink: 0 }} />
          <span style={{ fontFamily: "'Barlow', sans-serif", fontSize: 13, color: statusColor[voiceState], letterSpacing: "0.02em" }}>
            {statusLabel[voiceState]}
          </span>
        </div>

        {/* Volume bar — only when user is speaking */}
        <div style={{ width: 200, height: 3, background: "rgba(255,255,255,0.06)", borderRadius: 2, marginBottom: 20, overflow: "hidden", opacity: voiceState === "active" ? 1 : 0, transition: "opacity 0.3s" }}>
          <div style={{ height: "100%", width: `${volumeLevel}%`, background: "linear-gradient(90deg, #22C55E, #00A8E8)", borderRadius: 2, transition: "width 0.08s ease" }} />
        </div>

        {/* Transcript scroll area */}
        <div
          style={{ width: "100%", flex: 1, padding: "0 20px", display: "flex", flexDirection: "column", gap: 8, overflowY: "auto", maxHeight: 220 }}
        >
          {transcriptLog.length === 0 && (voiceState === "active" || voiceState === "agent-speaking" || voiceState === "connecting") && (
            <div style={{ textAlign: "center", fontFamily: "'Barlow', sans-serif", fontSize: 13, color: "#374151", fontStyle: "italic", padding: "16px 0" }}>
              {voiceState === "connecting" ? "Connecting, please wait…" : "The assistant will greet you shortly…"}
            </div>
          )}

          {transcriptLog.map((entry, i) => (
            <div key={i} style={{ display: "flex", flexDirection: entry.role === "user" ? "row-reverse" : "row", gap: 8, alignItems: "flex-end" }}>
              <div style={{
                maxWidth: "80%",
                padding: "8px 13px",
                borderRadius: entry.role === "user" ? "14px 14px 3px 14px" : "3px 14px 14px 14px",
                background: entry.role === "user" ? "rgba(0,168,232,0.15)" : "rgba(255,255,255,0.04)",
                border: entry.role === "user" ? "1px solid rgba(0,168,232,0.25)" : "1px solid rgba(255,255,255,0.07)",
                fontFamily: "'Barlow', sans-serif",
                fontSize: 12.5,
          ...
          {agentTranscript && voiceState === "agent-speaking" && (
            <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
              <div style={{ maxWidth: "80%", padding: "8px 13px", borderRadius: "3px 14px 14px 14px", background: "rgba(0,168,232,0.08)", border: "1px solid rgba(0,168,232,0.18)", fontFamily: "'Barlow', sans-serif", fontSize: 12.5, color: "#93C5FD", lineHeight: 1.55, fontStyle: "italic" }}>
                {agentTranscript}<span style={{ opacity: 0.5 }}>▋</span>
              </div>
            </div>
          )}
          ...
          {userTranscript && (
            <div style={{ display: "flex", flexDirection: "row-reverse", gap: 8, alignItems: "flex-end" }}>
              <div style={{ maxWidth: "80%", padding: "8px 13px", borderRadius: "14px 14px 3px 14px", background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.2)", fontFamily: "'Barlow', sans-serif", fontSize: 12.5, color: "#86EFAC", lineHeight: 1.55, fontStyle: "italic" }}>
                {userTranscript}
              </div>
            </div>
          )}
        ...
        <button
          onClick={handleClose}
          style={{ marginTop: 20, padding: "11px 36px", background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 50, fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", color: "#EF4444", cursor: "pointer", transition: "background 0.2s" }}
...
// ─── Share Modal ──────────────────────────────────────────────────────────────
function ShareModal({ onClose, conversationSummary: _conversationSummary }: ShareModalProps) {
  const [email, setEmail] = useState("");
  const [sent, setSent] = useState(false);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "flex-end", backdropFilter: "blur(8px)" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", background: "linear-gradient(180deg, #0F1318 0%, #080A0E 100%)", border: "1px solid rgba(0,168,232,0.15)", borderBottom: "none", borderRadius: "20px 20px 0 0", padding: "8px 20px 44px", fontFamily: "'Barlow Condensed', sans-serif" }}>
        <div style={{ width: 40, height: 3, background: "rgba(255,255,255,0.12)", borderRadius: 2, margin: "12px auto 24px" }} />
        <h3 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 800, fontSize: 22, letterSpacing: "0.06em", textTransform: "uppercase", color: "#E8ECF0", marginBottom: 4 }}>Continue Your Journey</h3>
        <p style={{ fontFamily: "'Barlow', sans-serif", fontSize: 13, color: "#4B5563", marginBottom: 24, lineHeight: 1.5 }}>Send this conversation to your device to explore further at home.</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <input type="email" placeholder="your@email.com" value={email} onChange={(e) => setEmail(e.target.value)}
            style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: "13px 16px", fontFamily: "'Barlow', sans-serif", fontSize: 14, color: "#E8ECF0", outline: "none", width: "100%", boxSizing: "border-box" }} />
          <button onClick={handleSend} disabled={!email || sent}
            style={{ background: sent ? "rgba(0,168,232,0.4)" : "#00A8E8", color: "#fff", border: "none", borderRadius: 8, padding: "14px 0", fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 13, letterSpacing: "0.18em", textTransform: "uppercase", cursor: email && !sent ? "pointer" : "default" }}>
            {sent ? "✓  Sent Successfully" : "Send to Email"}
          </button>
        </div>
        <div style={{ marginTop: 20, display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "rgba(0,168,232,0.05)", border: "1px solid rgba(0,168,232,0.1)", borderRadius: 8 }}>
...
function MessageBubble({ message, isLatest }: { message: Message; isLatest: boolean }) {
  ...
  const renderContent = (text: string) =>
    text.split("\n").map((line, i, arr) => {
      ...
      return (
        <span key={i}>
          {parts.map((part, j) =>
            j % 2 === 1 ? <strong key={j} style={{ color: isUser ? "rgba(255,255,255,0.95)" : "#E8ECF0", fontWeight: 700 }}>{part}</strong> : part,
          )}
          {i < arr.length - 1 && <br />}
        </span>
      );
  ...
  return (
    <div style={{ display: "flex", flexDirection: isUser ? "row-reverse" : "row", alignItems: "flex-end", gap: 10, marginBottom: 16, animation: isLatest ? "msgIn 0.32s cubic-bezier(0.22,1,0.36,1) forwards" : "none", opacity: isLatest ? 0 : 1 }}>
      {!isUser && (
        <div style={{ width: 30, height: 30, borderRadius: "50%", background: "linear-gradient(135deg, #00A8E8 0%, #0066B3 100%)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 11, fontWeight: 900, color: "#fff", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.08em", border: "1px solid rgba(0,168,232,0.3)" }}>BYD</div>
      )}
      <div style={{ maxWidth: "78%", padding: isUser ? "10px 16px" : "12px 16px", borderRadius: isUser ? "16px 16px 4px 16px" : "4px 16px 16px 16px", background: isUser ? "linear-gradient(135deg, #00A8E8, #0066B3)" : "rgba(255,255,255,0.04)", border: isUser ? "none" : "1px solid rgba(255,255,255,0.07)", fontFamily: "'Barlow', sans-serif", fontSize: 13.5, lineHeight: 1.7, color: isUser ? "rgba(255,255,255,0.95)" : "#9CA3AF", boxShadow: isUser ? "0 4px 20px rgba(0,168,232,0.2)" : "none" }}>
        {renderContent(message.content)}
      </div>
      {isUser && (
        <div style={{ width: 28, height: 28, borderRadius: "50%", background: "rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "1px solid rgba(255,255,255,0.08)" }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
...
function SuggestionPills({ suggestions, onSelect, label }: { suggestions: string[]; onSelect: (s: string) => void; label: string }) {
  return (
    <div style={{ padding: "0 16px 12px", flexShrink: 0 }}>
      ...
      <div className="byd-suggestions-scroll" style={{ display: "flex", flexWrap: "nowrap", gap: 8, overflowX: "auto", overflowY: "hidden", WebkitOverflowScrolling: "touch" as any, paddingBottom: 4 }}>
        {suggestions.map((s) => (
          <button key={s} onClick={() => onSelect(s)}
            style={{ flex: "0 0 auto", whiteSpace: "nowrap", padding: "7px 13px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 20, fontFamily: "'Barlow', sans-serif", fontSize: 12, color: "#6B7280", cursor: "pointer", transition: "all 0.2s", lineHeight: 1.3, maxWidth: "80vw" }}
            onMouseEnter={(e) => { const el = e.currentTarget as HTMLButtonElement; el.style.background = "rgba(0,168,232,0.08)"; el.style.borderColor = "rgba(0,168,232,0.3)"; el.style.color = "#E8ECF0"; }}
            onMouseLeave={(e) => { const el = e.currentTarget as HTMLButtonElement; el.style.background = "rgba(255,255,255,0.03)"; el.style.borderColor = "rgba(255,255,255,0.07)"; el.style.color = "#6B7280"; }}
          >
            {s}
          </button>
        ))}
...
export default function BYDChatbot({ isOpen, onClose, carContext, suggestionContext }: BYDChatbotProps) {
  ...
  const [conversationHistory, setConversationHistory] = useState<ConversationHistoryItem[]>([]);
  const [hasStarted, setHasStarted] = useState(false);
  const [currentSuggestions, setCurrentSuggestions] = useState<string[]>(CONTEXT_SUGGESTIONS[suggestionContext ?? "default"]);
  const [showSuggestions, setShowSuggestions] = useState(true);
  ...
  useEffect(() => {
    if (suggestionContext) setCurrentSuggestions(CONTEXT_SUGGESTIONS[suggestionContext] ?? CONTEXT_SUGGESTIONS.default);
  }, [suggestionContext]);
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      const response = await fetch("https://byd-backend.omnisuiteai.com/api/chatbot", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: trimmed, conversationHistory, carContext: carContext ?? null }),
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();
      if (data.success && data.response) {
  ...
  const handleReset = () => {
    ...
    setCurrentSuggestions(CONTEXT_SUGGESTIONS[suggestionContext ?? "default"]);
    setTimeout(() => {
      setMessages([{ id: "welcome-reset", role: "assistant", content: `Fresh start! What would you like to explore about the ${carContext ?? "BYD range"}?`, timestamp: new Date() }]);
    }, 50);
  };

  const conversationSummary = messages.slice(0, 6).map((m) => `${m.role === "user" ? "Q" : "A"}: ${m.content.slice(0, 80)}`).join("\n");
  ...
  return (
    <>
      ...
      {/* Backdrop */}
      <div style={{ position: "fixed", inset: 0, zIndex: 900, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(6px)", animation: "overlayIn 0.25s ease forwards" }} onClick={onClose} />

      {/* Chat Panel */}
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 950, height: "91dvh", maxHeight: 720, background: "#080A0E", borderTop: "1px solid rgba(0,168,232,0.12)", borderLeft: "1px solid rgba(0,168,232,0.06)", borderRight: "1px solid rgba(0,168,232,0.06)", borderRadius: "20px 20px 0 0", display: "flex", flexDirection: "column", fontFamily: "'Barlow Condensed', sans-serif", animation: "chatPanelUp 0.4s cubic-bezier(0.22,1,0.36,1) forwards", overflow: "hidden" }}>
        <div style={{ height: 1, background: "linear-gradient(90deg, transparent, rgba(0,168,232,0.5), transparent)", flexShrink: 0 }} />

        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)", background: "rgba(8,10,14,0.9)", backdropFilter: "blur(20px)", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 38, height: 38, borderRadius: 10, background: "linear-gradient(145deg, #00A8E8, #004E8C)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "1px solid rgba(0,168,232,0.4)", boxShadow: "0 0 16px rgba(0,168,232,0.12)" }}>
              <svg width="20" height="14" viewBox="0 0 48 32" fill="none"><text x="4" y="25" fontFamily="'Barlow Condensed',sans-serif" fontWeight="900" fontSize="26" fill="white" letterSpacing="-1">BYD</text></svg>
            </div>
            <div>
              <div style={{ fontWeight: 800, fontSize: 15, letterSpacing: "0.1em", textTransform: "uppercase", color: "#E8ECF0", lineHeight: 1.1 }}>BYD Assistant</div>
              <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 3 }}>
                <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#22c55e", boxShadow: "0 0 6px rgba(34,197,94,0.6)" }} />
                <span style={{ fontFamily: "'Barlow', sans-serif", fontSize: 11, color: "#4B5563", fontWeight: 400 }}>
                  Online · BYD Fairfield{carContext && <span style={{ color: "#00A8E8", marginLeft: 4 }}>· {carContext}</span>}
                </span>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            ...
            <button
              ...
              style={{ background: "rgba(0,168,232,0.08)", border: "1px solid rgba(0,168,232,0.25)", borderRadius: 8, padding: "6px 12px", display: "flex", alignItems: "center", gap: 6, cursor: "pointer", fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 10, letterSpacing: "0.18em", textTransform: "uppercase", color: "#00A8E8", transition: "all 0.2s", animation: "voiceButtonPulse 3s ease-in-out infinite" }}
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
            ...
            {hasStarted && (
              <button onClick={() => setShowShare(true)} className="byd-icon-btn" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#4B5563", transition: "all 0.2s" }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
            ...
            {hasStarted && (
              <button onClick={handleReset} className="byd-icon-btn" title="New conversation" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#4B5563", transition: "all 0.2s" }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="23 4 23 10 17 10" /><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10" />
                </svg>
              </button>
            )}
            <button onClick={onClose} className="byd-icon-btn" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#4B5563", transition: "all 0.2s" }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
        ...
        {!hasStarted && (
          <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.04)", flexShrink: 0 }}>
            <div style={{ fontFamily: "'Barlow', sans-serif", fontSize: 10, color: "#374151", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8 }}>Where are you in your journey?</div>
            <div style={{ display: "flex", gap: 7, overflowX: "auto", paddingBottom: 2 }}>
              {JOURNEY_STAGES.map((stage) => (
                <button key={stage.label} onClick={() => sendMessage(stage.prompt)}
                  style={{ flexShrink: 0, padding: "7px 14px", background: "transparent", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 20, fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 10, letterSpacing: "0.15em", textTransform: "uppercase", color: "#6B7280", cursor: "pointer", transition: "all 0.2s" }}
                  onMouseEnter={(e) => { const el = e.currentTarget as HTMLButtonElement; el.style.borderColor = "rgba(0,168,232,0.4)"; el.style.color = "#00A8E8"; el.style.background = "rgba(0,168,232,0.07)"; }}
                  onMouseLeave={(e) => { const el = e.currentTarget as HTMLButtonElement; el.style.borderColor = "rgba(255,255,255,0.09)"; el.style.color = "#6B7280"; el.style.background = "transparent"; }}
                >
                  {stage.label}
                </button>
              ))}
            ...
            <div
              style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "rgba(0,168,232,0.06)", border: "1px solid rgba(0,168,232,0.12)", borderRadius: 10, cursor: "pointer" }}
              onClick={() => setShowVoice(true)}
            >
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg, #00A8E8, #0066B3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                  <path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z" /><path d="M19 10v2a7 7 0 0 1-14 0v-2" /><line x1="12" y1="19" x2="12" y2="23" /><line x1="8" y1="23" x2="16" y2="23" />
                </svg>
              </div>
              <div>
                <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 12, color: "#E8ECF0", letterSpacing: "0.06em" }}>Prefer to talk? Try our Voice Assistant</div>
                <div style={{ fontFamily: "'Barlow', sans-serif", fontSize: 11, color: "#4B5563", marginTop: 2 }}>Speak naturally — ask anything about any BYD model</div>
              </div>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#00A8E8" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{ marginLeft: "auto", flexShrink: 0 }}>
        ...
        {/* Messages */}
        <div className="byd-scroll" style={{ flex: 1, overflowY: "auto", padding: "20px 16px 8px" }}>
        ...
        {/* Suggestions */}
        {showSuggestions && !isLoading && messages.length > 0 && (
          <SuggestionPills label={hasStarted ? "Follow-up questions" : "Common questions"} suggestions={currentSuggestions} onSelect={sendMessage} />
        )}

        {/* Input bar */}
        <div style={{ padding: "10px 14px 18px", borderTop: "1px solid rgba(255,255,255,0.05)", background: "rgba(8,10,14,0.98)", flexShrink: 0 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
            {/* Inline mic button */}
            <button
              onClick={() => setShowVoice(true)}
              title="Voice call"
              style={{ width: 40, height: 40, borderRadius: 10, border: "1px solid rgba(0,168,232,0.2)", background: "rgba(0,168,232,0.07)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, transition: "all 0.2s", color: "#00A8E8" }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(0,168,232,0.16)"; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(0,168,232,0.07)"; }}
            >
            ...
            <div
              style={{ flex: 1, display: "flex", gap: 10, alignItems: "flex-end", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "10px 10px 10px 16px", transition: "border-color 0.2s" }}
              onFocusCapture={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(0,168,232,0.4)"; }}
              onBlurCapture={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(255,255,255,0.08)"; }}
            >
              <textarea
                ...
                placeholder={carContext ? `Ask about the ${carContext}…` : "Ask anything about BYD vehicles…"}
                rows={1}
                style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontFamily: "'Barlow', sans-serif", fontSize: 14, color: "#E8ECF0", resize: "none", lineHeight: 1.55, maxHeight: 84, overflowY: "auto", paddingTop: 1 }}
                onInput={(e) => { const t = e.target as HTMLTextAreaElement; t.style.height = "auto"; t.style.height = `${Math.min(t.scrollHeight, 84)}px`; }}
              />
              <button
                className="byd-send"
                onClick={() => sendMessage(input)}
                disabled={!input.trim() || isLoading}
                style={{ width: 36, height: 36, borderRadius: 9, border: "none", background: input.trim() && !isLoading ? "#00A8E8" : "rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "center", cursor: input.trim() && !isLoading ? "pointer" : "default", transition: "all 0.2s", flexShrink: 0, boxShadow: input.trim() && !isLoading ? "0 4px 14px rgba(0,168,232,0.3)" : "none" }}
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={input.trim() && !isLoading ? "#fff" : "#374151"} strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
      ...
      {showShare && <ShareModal onClose={() => setShowShare(false)} conversationSummary={conversationSummary} />}
      {showVoice && <VoiceModal onClose={() => setShowVoice(false)} carContext={carContext} />}
    </>
  );
}

// ─── Floating FAB ─────────────────────────────────────────────────────────────
export function BYDChatbotButton({ carContext, suggestionContext }: { carContext?: string; suggestionContext?: keyof typeof CONTEXT_SUGGESTIONS }) {
  const [isOpen, setIsOpen] = useState(false);
  const [pulse, setPulse] = useState(true);
  ...
  return (
    <>
      ...
      <button
        onClick={() => setIsOpen(true)}
        style={{ position: "fixed", bottom: 100, right: 16, zIndex: 800, width: 52, height: 52, borderRadius: "50%", background: "linear-gradient(145deg, #00A8E8, #004E8C)", border: "1px solid rgba(0,168,232,0.5)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 20px rgba(0,168,232,0.35)", transition: "transform 0.2s, box-shadow 0.2s" }}
        onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.transform = "scale(1.08)"; (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 8px 28px rgba(0,168,232,0.5)"; }}
        onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.transform = "scale(1)"; (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 4px 20px rgba(0,168,232,0.35)"; }}
        title="Ask BYD Assistant"
      >
        {pulse && <div className="byd-fab-ring" />}
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round">
      ...
      <BYDChatbot isOpen={isOpen} onClose={() => setIsOpen(false)} carContext={carContext} suggestionContext={suggestionContext} />
    </>
  );
}
